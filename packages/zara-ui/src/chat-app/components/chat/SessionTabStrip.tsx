...
const TAB_COLLAPSED_KEY = "zara-tabs-collapsed"
...
export const SessionTabStrip: Component = () => {
  ...
  const [collapsed, setCollapsed] = createSignal(localStorage.getItem(TAB_COLLAPSED_KEY) !== "false")
  if (!tabs) return null

  const toggle = () => {
    const next = !collapsed()
    setCollapsed(next)
    localStorage.setItem(TAB_COLLAPSED_KEY, String(next))
  }
  ...
  return (
    <div class="session-tab-strip-wrapper">
      <button
        class="session-tab-toggle"
        onClick={toggle}
        title={collapsed() ? "Show tabs" : "Hide tabs"}
        aria-label={collapsed() ? "Show tabs" : "Hide tabs"}
      >
        <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
          <Show when={collapsed()} fallback={<path d="M4 6l4 4 4-4H4z" />}>
