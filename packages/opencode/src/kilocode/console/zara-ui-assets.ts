...
export namespace ZaraUiAssets {
  const log = Log.create({ service: "zara.ui.assets" })
  const prefix = "/assistant-ui"
  const base = "/assistant-ui/"
  ...
  function source() {
    return path.resolve(import.meta.dirname, "../../../../zara-ui")
  }
  ...
  async function build(app: string, out: string) {
    ...
    const run = runBuild(app, out).catch((err) => {
      log.warn("failed to build Zara UI assets", { err })
      return undefined
    })
    builds.set(app, run)
    return await run
  ...
  async function runBuild(app: string, out: string) {
    log.info("building Zara UI assets", { app })
    const proc = Bun.spawn([process.execPath, "run", "build"], {
      cwd: app,
      env: { ...process.env, KILO_ASSISTANT_UI_BASE: base },
      stdout: "pipe",
      stderr: "pipe",
      windowsHide: true,
    })
    ...
    if (code !== 0) throw new Error(`Zara UI build failed with exit code ${code}: ${stderr || stdout}`)
    if (!(await ready(out, true))) throw new Error("Zara UI build did not produce /assistant-ui assets")
    return out
  }
