...
export function Diff<T>(props: DiffProps<T>) {
  ...
  createEffect(() => {
    ...
    if (metadata) {
      ...
      instance.render({
        oldFile: { ...oldFile, name: oldFile?.name || "", contents: beforeContents, cacheKey: cacheKey(beforeContents) } as any,
        newFile: { ...newFile, name: newFile?.name || "", contents: afterContents, cacheKey: cacheKey(afterContents) } as any,
        lineAnnotations: annotations,
        containerWrapper: container,
      })
    }
